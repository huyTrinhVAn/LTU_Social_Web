...
export const getLikedPosts = async (req, res) => {
    ...
    try {
        const user = await User.findById(userId);
        if (!user) res.return(404).json({ error: " user not found" });
...
export const getFollowingPosts = async (req, res) => {
    ...
    try {
        const userId = req.user._id;
        const user = await User.findById(userId);
        if (!user) return res.status(404).json({ error: "User not fpund" });
        ...
        console.log("Error in getFollowingPosts controller.js: ", error);
...
export const getUserPosts = async (req, res) => {
    try {
        ...
        console.log("Erorr in getUserPosts controller: ", error);
        res.status(500).json({ error: " Internal server error" });
